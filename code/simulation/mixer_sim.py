...
def create_plant_sim_model(model_dir, sim_setup):
    ...
    equ_block = []
    bot_block = [
        '  annotation(uses(Modelica(version = "4.0.0")), experiment(StartTime = 0, StopTime = 1000, NumberOfIntervals = 500, Tolerance = 1e-6, Interval = 0.1),'
        '  Icon(graphics = {Text(origin = {2, -5}, textColor = {26, 95, 180}, extent = {{44, -51}, {-44, 51}}, textString = "P")}));',
        "end processPlant;",
    ]
    ...
    for m in mod_setup:
        if "source" in m:
            dek_block.append(
                f"    sourceModule {m}(redeclare package Medium = Medium);"
            )
        elif "sink" in m:
            ...
            dek_block.append(
                f"    filterModule {m}(redeclare package Medium = Medium);"
            )
        elif "distill" in m:
            dek_block.append(
                f"    distillModule {m}(redeclare package Medium = Medium);"
            )
        elif "bottling" in m:
            dek_block.append(
                f"    bottlingModule {m}(redeclare package Medium = Medium);"
            )
...
def write_anomaly_in_module(module, module_dict, reset=False):
    ...
    if reset:
        ...
        for line in model_code:
            if line.startswith("  parameter Boolean"):
                for anomaly in model_dict["faults"]:
                    if model_dict["faults"][anomaly] == True and line.startswith(
                        f"  parameter Boolean {anomaly}"
                    ):
    ...
    if not reset:
        print(f"anomalies have been updated for {module}.")
...
def data_cleaning(data_dir, sim_setup, modus):
    df = pd.read_csv(
        os.path.join(data_dir, sim_setup["ds_name"], f'{sim_setup["ds_name"]}.csv'),
        index_col=False,
    )
    ...
    drop_cols = [
        col for col in df.columns if "der(" in col or "level_to_boolean" in col
    ]
    ...
    if "_s" in modus:
        keep_cols += [
            col for col in df.columns if "state_" in col and not "monitoring" in col
        ]
    if "continuous" in modus:
        ...
        keep_cols += [
            ...
            if "sensor_continuous" in col and not "port" in col
        ]
        keep_cols += [
            col for col in df.columns if "volumeFlowRate" in col and "m_flow" in col
        ]
    ...
    df.to_csv(
        os.path.join(
            data_dir, sim_setup["ds_name"], f'{sim_setup["ds_name"]}_{modus}.csv'
        )
    )
    os.remove(
        os.path.join(data_dir, sim_setup["ds_name"], f'{sim_setup["ds_name"]}.csv')
...
def run_sim(
    ...
    filelist = [
        f
        for f in os.listdir(os.path.join(data_dir, sim_setup["ds_name"]))
        if not f.endswith(".csv")
    ]
    for f in filelist:
        os.remove(os.path.join(data_dir, sim_setup["ds_name"], f))
    os.rename(
        os.path.join(data_dir, sim_setup["ds_name"], "processPlant_res.csv"),
        os.path.join(data_dir, sim_setup["ds_name"], f'{sim_setup["ds_name"]}.csv'),
    )
    ...
    print(
        f"simulation finished. data is saved in {os.path.join(data_dir, sim_setup['ds_name'])}"
    )
